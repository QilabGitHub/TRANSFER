...
% Channel 1: GFP
% Channel 2: mCherry
% Channel 3: mCherry (for identifying ROI)
% The user will be asked to manually select an ROI for the cell to be
% analyzed. For mCherry and GFP channels, a threshold value will be calculated based on 
% the pixel intensity statistics within the cell ROI. A binary mask will 
% then be created by applying thresholding in the mCherry images.
% Values lower than the threshold will be 
...
% revised by Yanyu Zhu, 2-11-2025, to calculate the intensity of channel 1,2,3 and area of
% the ROI selected
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
OMM_thres_factor = 2;           % * STD, thresholding factor for mCherry
RNA_Cas_thres_factor = 0.4;        % * STD, thresholding factor for GFP

% Define the filename for the output data
table_name = 'intensity_analysis-OMM2-RNA0.4.csv';
...
channel_seq = readtable ('Channel_sequence_intensity.xlsx');
channel_name = channel_seq.Channel_name;

% Define the reading sequence of the channels
RNA_idx = find(ismember(channel_name, 'GFP'));
Cas_ind = find(ismember(channel_name, 'mCherry'));
OMM_ind = find(ismember(channel_name, 'mCherry'));
channel_ind = [RNA_idx; Cas_ind; OMM_ind];

% Find all 'tif' files in the folder
imagefiles = dir('*.tif');      
nfiles = length(imagefiles);    % Number of files found
pixel=0.183; % revised by Yanyu, input your pixel in um
...
Intensity_OMM=zeros(nfiles,1);
Intensity_RNA=zeros(nfiles,1);
Intensity_dCas13=zeros(nfiles,1);
ROI_area=zeros(nfiles,1);
...
for jj = 1: 1: nfiles
    ...
    RNA_Img = ImgMatrix_sub(:,:,1);
    RNA_roi=RNA_Img(bw);
    dCas13_Img = ImgMatrix_sub(:,:,2);
    dCas13_roi=dCas13_Img(bw);
    ...
    Inte_intensity_OMM=sum(ROI_Img_Vec);
    Inte_intensity_RNA=sum(RNA_roi);
    Inte_intensity_dCas13=sum(dCas13_roi);
    ...
    Intensity_OMM(jj)=Inte_intensity_OMM;  
    Intensity_RNA(jj)=Inte_intensity_RNA; 
    Intensity_dCas13(jj)=Inte_intensity_dCas13; 
    ROI_area(jj)=area*pixel*pixel;
...
output_table = table (RNA_colocal, Cas_colocal, RNA_background, Cas_background,Intensity_RNA,Intensity_dCas13,Intensity_OMM,ROI_area, 'RowNames', filenames);
